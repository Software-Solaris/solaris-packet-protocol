...
static retval_t posixQueueSend(void *p_q, const void *p_item, uint32_t timeoutMs)
{
...
static retval_t posixQueueRecv(void *p_q, void *p_item, uint32_t timeoutMs)
{
    PosixQueue_t *p_queue = (PosixQueue_t *)p_q;
    pthread_mutex_lock(&p_queue->lock);
...
static retval_t posixMutexLock(void *p_m, uint32_t timeoutMs)
{
    (void)timeoutMs;
    return (pthread_mutex_lock((pthread_mutex_t *)p_m) == 0) ? SPP_OK : SPP_ERROR;
}

static retval_t posixMutexUnlock(void *p_m)
{
    return (pthread_mutex_unlock((pthread_mutex_t *)p_m) == 0) ? SPP_OK : SPP_ERROR;
}
...
static retval_t posixEventWait(void *p_e, uint32_t bits, spp_bool_t clearOnExit,
...
static retval_t posixEventSetFromIsr(void *p_e, uint32_t bits,
