...
static SPP_RetVal_t freertosQueueSend(void *p_q, const void *p_item, spp_uint32_t timeoutMs)
{
    BaseType_t ret = xQueueSend((QueueHandle_t)p_q, p_item, msToTicks(timeoutMs));
    return (ret == pdTRUE) ? K_SPP_OK : K_SPP_ERROR;
}

static SPP_RetVal_t freertosQueueRecv(void *p_q, void *p_item, spp_uint32_t timeoutMs)
{
...
static SPP_RetVal_t freertos_mutexLock(void *p_m, spp_uint32_t timeoutMs)
{
    BaseType_t ret = xSemaphoreTake((SemaphoreHandle_t)p_m, msToTicks(timeoutMs));
    return (ret == pdTRUE) ? K_SPP_OK : K_SPP_ERROR;
}

static SPP_RetVal_t freertos_mutexUnlock(void *p_m)
{
...
static SPP_RetVal_t freertosEventWait(void *p_e, spp_uint32_t bits,
                                   ...
                                   spp_uint32_t *p_actualBits)
{
    ...
    if (waitAll)
    {
        return ((result & bits) == bits) ? K_SPP_OK : K_SPP_ERROR_TIMEOUT;
    }
    return ((result & bits) != 0U) ? K_SPP_OK : K_SPP_ERROR_TIMEOUT;
}

static SPP_RetVal_t freertosEventSetFromIsr(void *p_e, spp_uint32_t bits,
                                         spp_uint32_t *p_prev,
                                         spp_bool_t   *p_yield)
{
