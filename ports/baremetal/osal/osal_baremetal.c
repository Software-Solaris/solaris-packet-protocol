...
static retval_t bmQueueSend(void *p_q, const void *p_item, spp_uint32_t timeoutMs)
{
    (void)timeoutMs;
    BmQueue_t *p_queue = (BmQueue_t *)p_q;
    if (p_queue->count >= p_queue->capacity) return SPP_ERROR;
...
static retval_t bmQueueRecv(void *p_q, void *p_item, spp_uint32_t timeoutMs)
{
    (void)timeoutMs;
    BmQueue_t *p_queue = (BmQueue_t *)p_q;
    if (p_queue->count == 0U) return SPP_ERROR;
...
static retval_t bmMutexLock(void *p_m, spp_uint32_t t)   { (void)p_m; (void)t; return SPP_OK; }
static retval_t bmMutexUnlock(void *p_m)                 { (void)p_m; return SPP_OK; }
...
static retval_t bmEventWait(void *p_e, spp_uint32_t bits, spp_bool_t clear,
                             ...
                             spp_uint32_t *p_actual)
{
    ...
    if (matched == 0U) return SPP_ERROR_TIMEOUT;
    if (clear) *p_bits &= ~bits;
    return SPP_OK;
}

static retval_t bmEventSetFromIsr(void *p_e, spp_uint32_t bits,
