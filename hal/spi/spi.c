 ...
 */
__attribute__((weak)) retval_t SPI_Init(spi_handle_t* handle, const spi_config_t* config)
{
    if (handle == NULL || config == NULL) {
        return SPP_ERROR_NULL_POINTER;
    }
    
    // Default implementation - just copy config
    handle->spi_id = 0;
    memcpy(&handle->config, config, sizeof(spi_config_t));
 ...
 */
__attribute__((weak)) retval_t SPI_Transmit(spi_handle_t* handle, const uint8_t* tx_data, 
                                           uint16_t length, uint32_t timeout_ms)
{
    if (handle == NULL || tx_data == NULL) {
        return SPP_ERROR_NULL_POINTER;
    }
    ...
    // Default implementation - just return OK (no actual transmission)
    (void)length;
    (void)timeout_ms;
 ...
 */
__attribute__((weak)) retval_t SPI_Receive(spi_handle_t* handle, uint8_t* rx_data, 
                                          uint16_t length, uint32_t timeout_ms)
{
