...
static SPP_RetVal_t ICM20948_setBank(void *p_spi, ICM20948_RegBank_t regBank)
{
    ...
    switch (regBank)
    {
        case K_ICM20948_REG_BANK_0: regBankSel.bits.bankSel = 0U; break;
        case K_ICM20948_REG_BANK_1: regBankSel.bits.bankSel = 1U; break;
        case K_ICM20948_REG_BANK_2: regBankSel.bits.bankSel = 2U; break;
        case K_ICM20948_REG_BANK_3: regBankSel.bits.bankSel = 3U; break;
        default: return K_SPP_ERROR;
    }
                                       ...
                                       const spp_uint8_t *p_bytes, spp_uint8_t len)
{
    ...
    {
        ret = ICM20948_writeReg(p_spi, K_ICM20948_REG_MEM_BANK_SEL,
...
static SPP_RetVal_t ICM20948_lpWakeCycle(void *p_data)
{
    ...
    if (p_spi == NULL) { return K_SPP_ERROR_NULL_POINTER; }
...
SPP_RetVal_t ICM20948_loadDmp(void *p_data)
{
    ...
    for (spp_uint16_t i = 0U; i < firmwareSize; i++, addr++)
    {
        ...
        if (readValue != p_firmware[i]) { return K_SPP_ERROR; }
    }
...
SPP_RetVal_t ICM20948_configDmpInit(void *p_data)
{
    ...
    if (whoAmIValue != K_ICM20948_WHO_AM_I_VALUE) { return K_SPP_ERROR; }
...
void ICM20948_checkFifoData(void *p_data)
{
...

    {
        ...
        if ((intStatus & 0x02U) != 0U)
        {
            ...
            {
                spp_uint16_t fifoCount =
                    ((spp_uint16_t)txRxData[1] << 8) | txRxData[2];
                ...
                {
                    spp_uint16_t numPackets =
                        fifoCount / K_ICM20948_DMP_PACKET_SIZE_BYTES;
...
{
    ICM20948_ServiceCtx_t       *ctx = (ICM20948_ServiceCtx_t *)p_ctx;
    const ICM20948_ServiceCfg_t *cfg = (const ICM20948_ServiceCfg_t *)p_cfg;

    ctx->p_spi = SPP_HAL_spiGetHandle(cfg->spiDevIdx);

    SPP_LOGI(K_ICM20948_LOG_TAG, "Service init (spiDevIdx=%u)", cfg->spiDevIdx);
    return K_SPP_OK;
}
