/*
 ...
 Modified    : Renamed file and public API symbols to SPP_EXTERNAL_ENCRYPTION_*
 ==============================================================================
 */

#ifndef SPP_EXTERNAL_ENCRYPTION_H_
#define SPP_EXTERNAL_ENCRYPTION_H_
...
Encryption/decryption of a single block with Rijndael
 -----------------------------------------------------------------------------*/
#if MICRO_RJNDL
void SPP_EXTERNAL_ENCRYPTION_cipher( const uint8_t* key,          /* encryption/decryption key    */
...
Main functions for ECB-AES block ciphering
 -----------------------------------------------------------------------------*/
#if ECB
void SPP_EXTERNAL_ENCRYPTION_ecbEncrypt( const uint8_t* key,     /* encryption key               */
                      const void* pntxt,      /* plaintext input              */
                      const size_t ptextLen,  /* length of plaintext          */
                      void* crtxt );          /* ciphertext result            */

char SPP_EXTERNAL_ENCRYPTION_ecbDecrypt( const uint8_t* key,     /* decryption key               */
...
Main functions for CBC-AES block ciphering
 -----------------------------------------------------------------------------*/
#if CBC
char SPP_EXTERNAL_ENCRYPTION_cbcEncrypt( const uint8_t* key,     /* encryption key               */
                      const uint8_t iVec[16], /* initialization vector        */
                      const void* pntxt,      /* plaintext input              */
                      const size_t ptextLen,  /* length of plaintext          */
                      void* crtxt );          /* ciphertext result            */

char SPP_EXTERNAL_ENCRYPTION_cbcDecrypt( const uint8_t* key,     /* decryption key               */
                      const uint8_t iVec[16], /* initialization vector        */
                      const void* crtxt,      /* ciphertext input             */
                      const size_t crtxtLen,  /* length of ciphertext         */
                      void* pntxt );          /* plaintext result             */
...
Main functions for CFB-AES block ciphering
 -----------------------------------------------------------------------------*/
#if CFB
void SPP_EXTERNAL_ENCRYPTION_cfbEncrypt( const uint8_t* key,     /* encryption key               */
                      const uint8_t iVec[16], /* initialization vector        */
                      const void* pntxt,      /* plaintext input              */
                      const size_t ptextLen,  /* length of plaintext          */
                      void* crtxt );          /* ciphertext result            */

void SPP_EXTERNAL_ENCRYPTION_cfbDecrypt( const uint8_t* key,     /* decryption key               */
                      const uint8_t iVec[16], /* initialization vector        */
                      const void* crtxt,      /* ciphertext input             */
                      const size_t crtxtLen,  /* length of ciphertext         */
                      void* pntxt );          /* plaintext result             */
...
Main functions for OFB-AES block ciphering
 -----------------------------------------------------------------------------*/
#if OFB
void SPP_EXTERNAL_ENCRYPTION_ofbEncrypt( const uint8_t* key,     /* encryption key               */
                      const uint8_t iVec[16], /* initialization vector        */
                      const void* pntxt,      /* plaintext input              */
                      const size_t ptextLen,  /* length of plaintext          */
                      void* crtxt );          /* ciphertext result            */

void SPP_EXTERNAL_ENCRYPTION_ofbDecrypt( const uint8_t* key,     /* decryption key               */
                      const uint8_t iVec[16], /* initialization vector        */
                      const void* crtxt,      /* ciphertext input             */
                      const size_t crtxtLen,  /* length of ciphertext         */
                      void* pntxt );          /* plaintext result             */
...
Main functions for XTS-AES block ciphering
 -----------------------------------------------------------------------------*/
#if XTS
char SPP_EXTERNAL_ENCRYPTION_xtsEncrypt( const uint8_t* keys,    /* encryption key pair          */
                      const uint8_t* tweak,   /* tweak value (unit/sector ID) */
                      const void* pntxt,      /* plaintext input              */
                      const size_t ptextLen,  /* length of plaintext          */
                      void* crtxt );          /* ciphertext result            */

char SPP_EXTERNAL_ENCRYPTION_xtsDecrypt( const uint8_t* keys,    /* decryption key pair          */
                      const uint8_t* tweak,   /* tweak value (unit/sector ID) */
                      const void* crtxt,      /* ciphertext input             */
                      const size_t crtxtLen,  /* length of ciphertext         */
                      void* pntxt );          /* plaintext result             */
...
Main functions for CTR-AES block ciphering
 -----------------------------------------------------------------------------*/
#if CTR_NA
void SPP_EXTERNAL_ENCRYPTION_ctrEncrypt( const uint8_t* key,     /* encryption key               */
                      const uint8_t* iv,      /* initialization vector/ nonce */
                      const void* pntxt,      /* plaintext input              */
                      const size_t ptextLen,  /* length of plaintext          */
                      void* crtxt );          /* ciphertext result            */

void SPP_EXTERNAL_ENCRYPTION_ctrDecrypt( const uint8_t* key,     /* decryption key               */
                      const uint8_t* iv,      /* initialization vector/ nonce */
                      const void* crtxt,      /* ciphertext input             */
                      const size_t crtxtLen,  /* length of ciphertext         */
                      void* pntxt );          /* plaintext result             */
...
Main functions for SIV-AES block ciphering
 -----------------------------------------------------------------------------*/
#if SIV
void SPP_EXTERNAL_ENCRYPTION_sivEncrypt( const uint8_t* keys,    /* encryption key pair          */
...
char SPP_EXTERNAL_ENCRYPTION_sivDecrypt( const uint8_t* keys,    /* decryption key pair          */
...
Main functions for GCM-AES block ciphering
 -----------------------------------------------------------------------------*/
#if GCM
void SPP_EXTERNAL_ENCRYPTION_gcmEncrypt( const uint8_t* key,     /* encryption key               */
...
char SPP_EXTERNAL_ENCRYPTION_gcmDecrypt( const uint8_t* key,     /* decryption key               */
                      const uint8_t* nonce,   /* nonce or IV (init. vector)   */
                      const void* aData,      /* added authentication data    */
                      const size_t aDataLen,  /* length of AAD (auth. data)   */
...
Main functions for CCM-AES block ciphering
 -----------------------------------------------------------------------------*/
#if CCM
void SPP_EXTERNAL_ENCRYPTION_ccmEncrypt( const uint8_t* key,     /* encryption key               */
                      const uint8_t* nonce,   /* nonce or IV (init. vector)   */
                      const void* aData,      /* added authentication data    */
                      const size_t aDataLen,  /* length of AAD (auth. data)   */
                      const void* pntxt,      /* plaintext input              */
...
char SPP_EXTERNAL_ENCRYPTION_ccmDecrypt( const uint8_t* key,     /* decryption key               */
                      const uint8_t* nonce,   /* nonce or IV (init. vector)   */
                      const void* aData,      /* added authentication data    */
                      const size_t aDataLen,  /* length of AAD (auth. data)   */
                      const void* crtxt,      /* ciphertext + appended tag    */
...
Main functions for OCB-AES block ciphering
 -----------------------------------------------------------------------------*/
#if OCB
void SPP_EXTERNAL_ENCRYPTION_ocbEncrypt( const uint8_t* key,     /* encryption key               */
                      const uint8_t* nonce,   /* nonce or IV (init. vector)   */
                      const void* aData,      /* added authentication data    */
                      const size_t aDataLen,  /* length of AAD (auth. data)   */
                      const void* pntxt,      /* plaintext input              */
...
char SPP_EXTERNAL_ENCRYPTION_ocbDecrypt( const uint8_t* key,     /* decryption key               */
                      const uint8_t* nonce,   /* nonce or IV (init. vector)   */
                      const void* aData,      /* added authentication data    */
                      const size_t aDataLen,  /* length of AAD (auth. data)   */
                      const void* crtxt,      /* ciphertext + appended tag    */
...
Main functions for EAX-AES mode; more info at the bottom of this document.
 -----------------------------------------------------------------------------*/
#if EAX
void SPP_EXTERNAL_ENCRYPTION_eaxEncrypt( const uint8_t* key,     /* encryption key               */
...
char SPP_EXTERNAL_ENCRYPTION_eaxDecrypt( const uint8_t* key,     /* decryption key               */
                      const uint8_t* nonce,   /* arbitrary-size nonce array   */
#if EAXP
                      const size_t nonceLen,  /* length of provided nonce     */
#else
...
Main functions for GCM-SIV-AES block ciphering
 -----------------------------------------------------------------------------*/
#if GCM_SIV
void SPP_EXTERNAL_ENCRYPTION_gcmSivEncrypt( const uint8_t* key,     /* encryption key               */
...
char SPP_EXTERNAL_ENCRYPTION_gcmSivDecrypt( const uint8_t* key,     /* decryption key               */
                      const uint8_t* nonce,   /* provided 96-bit nonce        */
                      const void* aData,      /* added authentication data    */
                      const size_t aDataLen,  /* length of AAD (auth. data)   */
                      const void* crtxt,      /* ciphertext + appended tag    */
...
Main functions for AES key-wrapping
 -----------------------------------------------------------------------------*/
#if KWA
char SPP_EXTERNAL_ENCRYPTION_keyWrap( const uint8_t* kek,        /* key encryption key           */
...
char SPP_EXTERNAL_ENCRYPTION_keyUnwrap( const uint8_t* kek,      /* key encryption key           */
...
Main functions for FPE-AES; more info at the bottom of this page.
 -----------------------------------------------------------------------------*/
#if FPE
char SPP_EXTERNAL_ENCRYPTION_fpeEncrypt( const uint8_t* key,     /* encryption key               */
...
char SPP_EXTERNAL_ENCRYPTION_fpeDecrypt( const uint8_t* key,     /* decryption key               */
                      const uint8_t* tweak,   /* tweak bytes                  */
#if FF_X != 3
                      const size_t tweakLen,  /* length of tweak array        */
#endif
...
Main function for Poly1305-AES message authentication code
 -----------------------------------------------------------------------------*/
#if POLY1305
void SPP_EXTERNAL_ENCRYPTION_poly1305( const uint8_t* keys,       /* encryption/mixing key pair   */
...
Main function for AES Cipher-based Message Authentication Code
 -----------------------------------------------------------------------------*/
#if CMAC
void SPP_EXTERNAL_ENCRYPTION_cmac( const uint8_t* key,            /* encryption or cipher key     */
               const void* data,              /* input data buffer            */
               const size_t dataSize,         /* length of data in bytes      */
